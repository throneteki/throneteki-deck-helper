    ...
    return {
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                return deck.countDrawCards(function (card) {
...
var agendaRules = {
    ...
    '05045': {
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                var isScheme = function isScheme(card) {
                    return hasTrait(card, 'Scheme');
                };
                var schemePlots = deck.plotCards.filter(function (cardQuantity) {
                    return isScheme(cardQuantity.card);
                });
    ...
    '09045': {
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                return deck.countDrawCards(function (card) {
                    return card.type === 'character' && hasTrait(card, 'Maester');
                }) >= 12;
            }
        }]
    },
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                ...
                return attachmentNames.every(function (attachmentName) {
                    return deck.countCards(function (card) {
                        return card.name === attachmentName;
                    }) <= 1;
                });
            }
        }]
    ...
    '20051': {
        mayInclude: function mayInclude(card) {
            return card.type === 'character' && hasTrait(card, 'Fool') && !card.loyal;
        }
    },
    // The Many-Faced God
    '20052': {
        cannotInclude: function cannotInclude(card) {
            return card.type === 'plot' && hasTrait(card, 'Kingdom');
        }
    },
    ...
    '21030': {
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                return deck.plotCards.every(function (cardQuantity) {
                    return hasTrait(cardQuantity.card, 'Edict') || hasTrait(cardQuantity.card, 'Siege') || hasTrait(cardQuantity.card, 'War');
                });
            }
        }]
    },
    ...
    '23040': {
        cannotInclude: function cannotInclude(card) {
            return card.faction !== 'neutral' && card.type === 'location';
        }
    },
    // Draft Agendas
    // The Power of Wealth
    '00001': {
        ...
        rules: [{
            ...
            condition: function condition(deck) {
                ...
                var factions = new Set(outOfFactionCards.map(function (cardQuantity) {
